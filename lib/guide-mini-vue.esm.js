...
function createGetter(isReadongly = false, shallow = false) {
    return function get(target, key) {
        ...
        if (isObject(res)) {
            return isReadongly ? readonly(res) : reactive(res);
        }
        if (!isReadongly) {
            track(target, key);
        }
...
function createRenderer(options) {
    ...
    function processComponent(n1, n2, container, parentComponent, anchor) {
        if (!n1) {
            ...
            updateCommonent(n1, n2);
        }
    }
    function updateCommonent(n1, n2) {
        ...
        instance.uptate = effect(() => {
            if (!instance.isMounted) {
                ...
                if (next) {
                    ...
                    next.el = vnode.el;
                    updateCommonentPreRender(instance, next);
                }
                const { proxy } = instance;
...
function updateCommonentPreRender(instance, nextVNode) {
